...
namespace Tida.Canvas.Infrastructure.EditTools
{
    ...
    public partial class CopyEditTool : EditTool, IHaveMousePositionTracker
    {
        ...
        {
            //获取所有被选中的绘制对象;
            var selectedDrawObjects = CanvasContext.GetAllDrawObjects().Where(p => p.IsSelected).Union(_copiedDrawObjects.Where(p => p.IsSelected));
            ...
            foreach (var drawObject in selectedDrawObjects)
            {
                ...
                copyCell.DrawObjectMoveTool = DrawObjectMoveTools.FirstOrDefault(p => p.CheckDrawObjectMoveable(drawObject));
        ...
        private void RollBackToLastMouseDownPosition()
        {
            ...
            _copyEditDrawObjectCells.ForEach(p => { p.DrawObjectMoveTool.Move(p.CopiedDrawObject, MousePositionTracker.LastMouseDownPosition - MousePositionTracker.CurrentHoverPosition); });
        }
        ...
        private void ApplyCurrentPositionState(Vector2D thisMouseDownPosition)
        {
            ...
            _copyEditDrawObjectCells.ForEach(p => { p.DrawObjectMoveTool.Move(p.CopiedDrawObject, offset); });

            //本次拷贝的绘制对象入栈;
            var newCopiedDrawObjects = _copyEditDrawObjectCells.Select(p => p.CopiedDrawObject.Clone()).ToArray();
        ...
        protected override void OnMouseMove(MouseMoveEventArgs e)
        {
            ...
            //进行偏移变化;
            _copyEditDrawObjectCells.ForEach(p => { p.DrawObjectMoveTool.Move(p.CopiedDrawObject, position - MousePositionTracker.LastMouseDownPosition); });
