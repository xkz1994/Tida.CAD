...
namespace Tida.Canvas.Shell.MainPage
{
    ...
    public partial class MainDockingServiceImpl : IDockingService
    {
            ...
            Views.MainPage mainPage)
        {
            ...
            _mefDockingContainers.AddRange(dockingContainers.Where(p => p.Metadata.DockingServiceGUID == DockingService_Main).Select(p => new CreatedDockingContainer(p.Value, p.Metadata)));

            _mefDockingGroups.AddRange(dockingGroups.OrderBy(p => p.Metadata.Order).Where(p => _mefDockingContainers.Any(q => q.DockingContainerMetaData.GUID == p.Metadata.ContainerGUID)).Select(p => new CreatedDockingGroup(p.Value, p.Metadata)));

            _mefDockingPanes.AddRange(dockingPanes.Where(p => _mefDockingGroups.Any(q => q.DockingGroupMetaData.GUID == p.Metadata.InitPaneGroupGUID)).Select(p => new CreatedDockingPane(p.Value, p.Metadata)));
        ...
        {
            //寻找DockGroup是否已经存在;
            var radPaneGroup = _radDocking.SplitItems.Where(p => p is RadPaneGroup).Cast<RadPaneGroup>().FirstOrDefault(g => g.Tag is CreatedDockingGroup dockingGroup && dockingGroup.DockingGroupMetaData.GUID == dockingGroupGUID);
        ...
        {
            var radPane = _radDocking.Panes.FirstOrDefault(p =>
        ...
        private static RadPane GenericRadPane(CreatedDockingPane dockingPane)
        {
            ...
            dockingPane.DockingPane.IsHiddenChanged += delegate { radPane.IsHidden = dockingPane.DockingPane.IsHidden; };
