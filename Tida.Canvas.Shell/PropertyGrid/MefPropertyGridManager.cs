    ...
    public class MefPropertyGridManager
    {
            ...
            [ImportMany] IEnumerable<IEditorDescriptorProvider> mefEditorDescriptorProviders)
        {
            _mefInheritablePropertyDescriptors =
                mefPropertyDescriptors.Where(p => p.Metadata.Inheritable).Select(p => new CreatedPropertyDescriptor(p.Value, p.Metadata)).Union(mefPropertyDescriptorProviders.SelectMany(p => p.PropertyDescriptors)).ToArray();

            _mefNonInheritablePropertyDescriptors = mefPropertyDescriptors.Where(p => !p.Metadata.Inheritable).ToDictionary(p => p.Value.PropertyInfo, p => new CreatedPropertyDescriptor(p.Value, p.Metadata));

            _mefIgnoredPropertyDescriptors =
                mefIgnoredPropertyDescriptors.Where(p => p.Metadata.Inheritable).Select(p => new CreatedIgnoredPropertyDescriptor(p.Value, p.Metadata)).Union(mefIgnoredPropertyDescriptorProviders.SelectMany(p => p.IgnoredPropertyDescriptors)).ToArray();
            ...
            _mefEditorDescriptors = mefEditorDescriptors.Select(p => new CreatedEditorDescriptor(p.Value, p.Metadata)).Union(mefEditorDescriptorProviders.SelectMany(p => p.EditorDescriptors)).ToArray();
        }
        ...
        private IEnumerable<CustomAttributeBuilder> GetCustomAttributeBuilders(PropertyInfo originPropInfo)
        {
            ...
            if (relatedPropertyDescriptor != null)
            {
                ...
                yield return new CustomAttributeBuilder(
                    ...
                    new object[]
                    {
                        editorMetaData.EditorType, metaData.EditorTargetProperty,
