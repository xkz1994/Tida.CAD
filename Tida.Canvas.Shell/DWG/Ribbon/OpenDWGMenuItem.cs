    ...
    class OpenDWGMenuItem : IMenuItem
    {
        ...
        private void OpenDWG()
        {
            ...
            try
            {
                ...
                var layers = cadImg.Layers.GetLayersNames().Select(p => new CanvasLayerEx(Guid.NewGuid().ToString()) { LayerName = p }).ToArray();
                ...
                for (int i = 0; i < cadImg.Entities.Length; i++)
                {
                    ...
                    var drawObject = _cadBaseToDrawObjectConverters.Select(p => p.Convert(entity)).FirstOrDefault(p => p != null);
